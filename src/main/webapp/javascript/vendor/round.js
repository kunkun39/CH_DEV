//电视剧大图滚动
(function(jq) {
	var roundAbout = function(o, options, data) {
		this.parent = jq("#" + o);
		this.body = jq("body");
		if (this.parent.length <= 0) {
			return false;
		}
		this.options = jq.extend({}, roundAbout.options, options);
		if (typeof(data) !== 'object') return false;
		this.data = data || {};
		this.reset(options.width);
		//处理页面resize
		var _this = this;
		jq(window).resize(function() {
			_this.reset(options.width);
		});
	};
	roundAbout.prototype = {
		reset: function(option) {
			if (option) {
				this.options.width=option;
			} else if (parseInt(this.body.outerWidth()) > 1255) {
			...
			this.preLeft = 0; //第一张图片左偏移量
			this.nextLeft = this.options.width - 530; //第三张图片左偏移量
			this.preNLeft = -530;
			this.nextNLeft = this.options.width;
			this.pageNowLeft = (this.options.width - 640) / 2; //当前图片左偏移量
			this.drawContent();
		},
		drawContent: function() {
			...
			for (var item = 0, i = 1, l = this.data.length; item < l; ++item, ++i) {
				...
				unitItem += '<span class="slider-text">' + this.data[item].text + '</span>';
		...
		initContent: function() {
			...
			this.parent.find(".contentHolderUnit:nth-child(" + this.pageNow + ")").css({width: '640px',
				height: '270px',
				opacity: 1,
			...
			//			this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+") img").css({width:'550px',height:'250px'});
			...
			//			this.parent.find(".contentHolderUnit:nth-child("+pre+") img").css({height: '200px', width: '300px'});
			...
			//			this.parent.find(".contentHolderUnit:nth-child("+next+") img").css({height: '200px', width: '300px'});
